<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
<%@include file="include/header.jsp" %>

<div class="jumbotron text-center">
  <h1>STUDY SQUARE</h1> 
  <p>한발짝,한걸음,꿈을향해<span class="glyphicon glyphicon-pencil"></span></p> 
  <div align="right" ><font size="1"><span class="glyphicon glyphicon-bell"></span>운영시간: AM 10:00 ~PM 11:00<br><span class="glyphicon glyphicon-phone-alt"></span>문의:012-3456-7890</font></div>
</div>

    <div class="container">
        <div class="text-center">
            <h1>Hello world!</h1>
            <p>The time on the server is ${serverTime}.</p>
            s<br>
            s<br>
            s<br>
            s<br>
            s<br>s<br>
            s<br>s<br>
            s<br>
            s<br>s<br>s<br>s<br>s<br>
            s<br>s<br>s<br>s<br>s<br>s<br>s<br>s<br>s<br>s<br>
            s<br>s<br>s<br>s<br>s<br>s<br>s<br>s<br>s<br>s<br>s<br>s<br>s<br>s<br>
            s<br>s<br>s<br>s<br>s<br>s<br>s<br>s<br>s<br>s<br>s<br>s<br>s<br>s<br>s<br>s<br>s<br>s<br>
           	<button class="container btn btn-danger" type="button" > test</button>
            s
            s
            s
            s
            s
            s
            s
            s
            s
            s
            s
            s
            s
            s
            s
            s
            
        </div>
    </div>
<%@include file="include/footer.jsp" %>